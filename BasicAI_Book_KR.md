# PyTorch로 배우는 기초 AI v1.0

**초보자를 위한 필수 가이드**

*저자: jw*

이 책은 PyTorch로 기본적인 신경망 개념을 다룹니다. 각 장에는 다음이 포함됩니다:

- **완전한 소스 코드** - 독립적인 Python 스크립트
- **실제 출력** - 코드 실행 결과
- **핵심 개념** - 중요한 내용 정리
...
1. [신경망 분류](#chapter-1)
2. [신경망 회귀](#chapter-2)
3. [비트코인 가격 예측](#chapter-3)
4. [활성화 함수가 중요한 이유](#chapter-4)
5. [패턴 분류 (간소화된 MNIST)](#chapter-5)
6. [Softmax & Cross Entropy 심층 분석](#chapter-6)
7. [Top-K & Temperature 샘플링](#chapter-7)
8. [토큰 선택: Greedy vs 샘플링](#chapter-8)
9. [트랜스포머 학습 및 추론 흐름](#chapter-9)
...
*이진 분류: (x, y) 좌표로 점을 분류하기*

### 핵심 개념

- nn.Module 기본 클래스
- 이진 분류를 위한 BCELoss
- 확률 출력을 위한 Sigmoid 활성화
- SGD 옵티마이저
- 추론을 위한 model.eval()
...
STEP 3: Training
==================================================
Epoch  20: Loss = 0.5443, Accuracy = 60.0%
Epoch  40: Loss = 0.3194, Accuracy = 100.0%
Epoch  60: Loss = 0.1580, Accuracy = 100.0%
Epoch  80: Loss = 0.0964, Accuracy = 100.0%
Epoch 100: Loss = 0.0646, Accuracy = 100.0%
...
Test Results:
----------------------------------------
  Point (0.6, 0.7): probability = 0.460 → class 0
  Point (0.1, 0.9): probability = 0.015 → class 0
  Point (0.7, 0.8): probability = 0.708 → class 1
...
STEP 3: Training
==================================================
Epoch 100: MSE Loss = 437.7308, MAE = $195.99k
Epoch 200: MSE Loss = 202.4545, MAE = $122.01k
Epoch 300: MSE Loss = 101.2087, MAE = $77.80k
Epoch 400: MSE Loss = 65.8989, MAE = $71.01k
Epoch 500: MSE Loss = 56.1345, MAE = $67.18k
...
Predictions vs Actual (Training Data):
---------------------------------------------
  House 1: Actual $200k, Predicted $274.2k (error: +74.2k)
  House 2: Actual $300k, Predicted $274.2k (error: -25.8k)
  House 3: Actual $400k, Predicted $274.2k (error: -125.8k)
  House 4: Actual $220k, Predicted $274.2k (error: +54.2k)
  House 5: Actual $330k, Predicted $274.2k (error: -55.8k)
...
New House Price Predictions:
---------------------------------------------
  House: 1400 sqft, 3 rooms → Predicted: $274.2k
  House: 2500 sqft, 5 rooms → Predicted: $274.2k
  House: 800 sqft, 1 rooms → Predicted: $274.2k
...
STEP 3: Training
=======================================================
Epoch 100: MSE Loss = 2.0512, MAE = $1.31k
Epoch 200: MSE Loss = 1.9212, MAE = $1.30k
Epoch 300: MSE Loss = 1.9209, MAE = $1.30k
Epoch 400: MSE Loss = 1.9209, MAE = $1.30k
Epoch 500: MSE Loss = 1.9209, MAE = $1.30k
...
Predictions vs Actual (Training Data):
-------------------------------------------------------
  Day 1: Actual $25k, Predicted $23.7k (error: -1.3k)
...
-------------------------------------------------------
  Medium volume, Neutral sentiment
    ...
    → Predicted BTC: $37.0k ($36978)

  Very high volume, Extremely bullish
    Volume: $60B, Sentiment: +0.9
    → Predicted BTC: $75.4k ($75357)

  Very low volume, Very bearish
    Volume: $15B, Sentiment: -0.8
    → Predicted BTC: $16.9k ($16870)

  Medium-high volume, Moderately bullish
    Volume: $40B, Sentiment: +0.3
    → Predicted BTC: $49.8k ($49771)
...
## 제5장: 패턴 분류 (간소화된 MNIST)
### Pattern Classification (Simplified MNIST)

*이미지 대신 텍스트 패턴을 사용한 다중 클래스 분류*

### 핵심 개념

- 다중 클래스를 위한 CrossEntropyLoss
- Softmax 확률
- 예측을 위한 argmax
- 완전한 학습 워크플로우
- 신뢰도 점수
...
## 제6장: Softmax & Cross Entropy 심층 분석
### Softmax & Cross Entropy Deep Dive

*Softmax와 Cross Entropy 손실 단계별 시각화*

### 핵심 개념

- Softmax 공식: exp(xi) / sum(exp(xj))
- Cross entropy: -log(p_correct)
- 그래디언트: softmax(logits) - one_hot(target)
- 이 조합이 인기 있는 이유
- 트랜스포머 형태 (batch, seq, vocab)
...
## 제7장: Top-K & Temperature 샘플링
### Top-K & Temperature Sampling

*LLM이 텍스트 생성 다양성을 제어하는 방법*
...
Results from 10 samples: {'dog': 5, 'cat': 5}
...
## 제9장: 트랜스포머 학습 및 추론 흐름
### Transformer Training & Inference Flow

*트랜스포머와 유사한 모델의 전체 학습 및 생성 흐름*
...
| 작업 | 출력 활성화 | 손실 함수 | 옵티마이저 |
|------|-------------|-----------|------------|
| 이진 분류 | Sigmoid | BCELoss | SGD/Adam |
| 다중 클래스 분류 | Softmax | CrossEntropyLoss | Adam |
| 회귀 | 없음 (선형) | MSELoss | Adam |

### 학습 루프 패턴

```python
for epoch in range(epochs):
    predictions = model(X)           # 순전파
...
with torch.no_grad():                # 그래디언트 비활성화
    ...
    prediction = output.argmax()     # 클래스 얻기 (분류)
```
